...
struct thread
  {
    ...
    int recent_cpu;											/* Recent_cpu for the MLFQS scheduler. */
    ...
    struct lock * lock_seeking;         /* Pointer to the lock, which the thread is seeking. */
...
/* comparator */
bool th_before2(const struct list_elem *, const struct list_elem *, void *aux UNUSED);
